"""総務省参議院選挙データインポートスクリプト.

総務省が公開するXLS/XLSXファイルから参議院選挙区の全候補者データをインポートする。

Usage (Docker経由で実行):
    docker compose -f docker/docker-compose.yml exec sagebase \
        uv run python scripts/import_soumu_sangiin_election.py --election 26
    ...
    docker compose -f docker/docker-compose.yml exec sagebase \
        uv run python scripts/import_soumu_sangiin_election.py --election 26 --dry-run
...
async def run_import(election_number: int, dry_run: bool) -> bool:
    """1回分の選挙データをインポートする."""
    logger.info("=== 第%d回参議院選挙データインポート開始 ===", election_number)
    ...
    async with get_async_session() as session:
        ...
        data_source = SoumuSangiinElectionDataSource()
