"""Wikipedia参議院選挙データインポートスクリプト.

Wikipediaの選挙データから第1-27回（1947-2025年）の
参議院議員通常選挙当選者データをインポートする。
...
Usage (Docker経由で実行):
    docker compose -f docker/docker-compose.yml exec sagebase \
        uv run python scripts/import_wikipedia_sangiin_election.py --election 1
    ...
    docker compose -f docker/docker-compose.yml exec sagebase \
        uv run python scripts/import_wikipedia_sangiin_election.py --all --dry-run

データソース:
    Wikipedia 第N回参議院議員通常選挙
...
from src.infrastructure.importers.wikipedia_sangiin_election_data_source import (
    SUPPORTED_SANGIIN_ELECTIONS,
    WikipediaSangiinElectionDataSource,
)
...
async def run_import(election_number: int, dry_run: bool) -> bool:
    """1回分の参議院選挙データをインポートする."""
    logger.info(
        "=== 第%d回参議院選挙データインポート開始（Wikipedia） ===", election_number
    )
    ...
    async with get_async_session() as session:
        ...
        data_source = WikipediaSangiinElectionDataSource()
        ...
        logger.info("--- インポート結果 (第%d回参議院) ---", election_number)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Wikipedia参議院選挙データをインポート（第1-27回）",
        epilog=(
    ...
    parser.add_argument(
        "--election",
        type=int,
        choices=SUPPORTED_SANGIIN_ELECTIONS,
        help=(
            f"選挙回次（{min(SUPPORTED_SANGIIN_ELECTIONS)}"
            f"-{max(SUPPORTED_SANGIIN_ELECTIONS)}）"
        ),
        metavar="N",
    )
    parser.add_argument(
        ...
        dest="import_all",
        help=(
            f"全選挙（第{min(SUPPORTED_SANGIIN_ELECTIONS)}回"
            f"〜第{max(SUPPORTED_SANGIIN_ELECTIONS)}回）をインポート"
        ),
    )
    parser.add_argument(
        "--dry-run",
    ...
    if args.import_all:
        election_numbers = SUPPORTED_SANGIIN_ELECTIONS
    else:
