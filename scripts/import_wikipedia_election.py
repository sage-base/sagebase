"""Wikipedia衆議院選挙データインポートスクリプト.

Wikipediaの選挙データから第1-44回（1890-2005年）の
衆議院選挙当選者データをインポートする。

第1-40回: 中選挙区制（大選挙区制含む）の当選者テーブル
第41-44回: 小選挙区比例代表並立制のテンプレート

Usage (Docker経由で実行):
    docker compose -f docker/docker-compose.yml exec sagebase \
        uv run python scripts/import_wikipedia_election.py --election 41
    ...
    docker compose -f docker/docker-compose.yml exec sagebase \
        uv run python scripts/import_wikipedia_election.py --all --dry-run

データソース:
    Wikipedia 第N回衆議院議員総選挙
...
async def run_import(election_number: int, dry_run: bool) -> bool:
    """1回分の選挙データをインポートする."""
    logger.info(
        "=== 第%d回衆議院選挙データインポート開始（Wikipedia） ===", election_number
    )
    ...
    async with get_async_session() as session:
        ...
        data_source = WikipediaElectionDataSource()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Wikipedia衆議院選挙データをインポート（第1-44回）",
        epilog=(
