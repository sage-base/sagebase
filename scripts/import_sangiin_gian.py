"""smartnews-smri 参議院議案データ（gian.json）インポートスクリプト.

使い方:
    # GitHubから自動ダウンロードしてインポート
    docker compose exec app python scripts/import_sangiin_gian.py

    # ローカルファイルを指定してインポート
    docker compose exec app python scripts/import_sangiin_gian.py /path/to/gian.json
...
SMRI_RAW_URL = (
    ...
    "smartnews-smri/house-of-councillors/main/data/gian.json"
)


def fetch_gian_json() -> Path:
    """GitHubからgian.jsonをダウンロードして一時ファイルパスを返す."""
...
async def _run_import(file_path: Path, batch_size: int, conference_name: str) -> None:
    async with get_async_session() as session:
        ...
        submitter_repo = ProposalSubmitterRepositoryImpl(session)
        use_case = ImportSangiinGianUseCase(
            proposal_repository=proposal_repo,
            proposal_submitter_repository=submitter_repo,
        )
        ...
        logger.info("提出者: %d", result.submitters_created)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="smartnews-smri 参議院議案データ（gian.json）をインポート"
    )
    parser.add_argument(
        "file_path",
        type=Path,
        nargs="?",
        default=None,
        help="gian.json のパス（省略時はGitHubから自動取得）",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
