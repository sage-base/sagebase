...
Sagebaseのリポジトリ実装には、歴史的経緯から3種類のモデルパターンが混在しています。ADR 0003（リポジトリパターン採用）策定時点ではSQLAlchemy ORMでの統一を想定していましたが、その後の実装過程でPydanticモデルや動的モデルが導入され、現在は以下の3パターンが共存しています：
