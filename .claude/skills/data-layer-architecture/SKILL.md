...
description: 5層データアーキテクチャ設計。PG Bronze（LLM抽出ログ層）、PG Gold（確定データ層）、BQ Source（sagebase_source）、BQ Vault/Main（将来）。LLM抽出結果の履歴管理と人間修正の保護を実現。抽出処理の実装、ExtractionLogの使用、is_manually_verifiedフラグの扱いに関するガイダンスを提供。
---

# Data Layer Architecture（5層構成）

## Purpose
LLM抽出結果と確定データを分離し、BigQueryへミラーリングする5層アーキテクチャの設計ガイド。AIの抽出履歴を保持しながら、人間の修正を保護する。
...
```
┌─────────────────────────────────────────────────────────────┐
│              PG Bronze Layer（抽出ログ層）                    │
│                                                              │
│  - LLM抽出結果を追記専用（Immutable）で保存                   │
│  - 精度分析・トレーサビリティのための履歴                      │
│  - テーブル: extraction_logs, extracted_*                     │
└─────────────────────────────────────────────────────────────┘
                              │
                              │ is_manually_verified = false の場合のみ反映
                              ▼
┌─────────────────────────────────────────────────────────────┐
│              PG Gold Layer（確定データ層）                    │
│                                                              │
│  - アプリケーションが参照する唯一の正解データ                   │
│  - 人間の修正が優先される                                     │
│  - テーブル: politicians, proposals, speakers, etc.          │
└─────────────────────────────────────────────────────────────┘
                              │
                              │ export-to-bq でミラーリング
                              ▼
┌─────────────────────────────────────────────────────────────┐
│              BQ Source Layer（sagebase_source）               │
│                                                              │
│  - PG Gold のミラーリングデータセット                         │
│  - SOURCE_TABLES で定義された23テーブル                       │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼ （将来）
┌─────────────────────────────────────────────────────────────┐
│  BQ Vault（sagebase_vault） / BQ Main（sagebase）            │
└─────────────────────────────────────────────────────────────┘
```
