...
中間テーブル（ジャンクションテーブル）のシード作成時：
- [ ] **FK NULLレコードの網羅チェック**: 親テーブルのFK=NULLレコードを全件確認し、中間テーブルで表現すべきものがないかチェック（NULLだった理由が「1対1FKでは表現不能」だった場合、中間テーブルで表現可能になっている）
- [ ] **load-seeds.shの更新**: SEED_FILES配列への追加 + 既存DB向け個別チェックの追加
