...
日本語の政治データを扱う際、全角/半角文字の混在が頻繁に発生します。データソースごとに表記が統一されていないため、文字列比較や分割処理の前に正規化を行ってください。

### よくある全角/半角混在パターン

| 文字種 | 全角 | 半角 | 出現箇所の例 |
|--------|------|------|-------------|
| セミコロン | `；` | `;` | 会派名の区切り文字（SMRI衆議院データ） |
| 括弧 | `（）` | `()` | 政党名の補足表記 |
| 数字 | `１２３` | `123` | 回次番号、議案番号 |
| スペース | `　`（全角） | ` `（半角） | 会派名の前後、人名の姓名間 |
| 中点 | `・` | — | 連立会派名の区切り（常に全角） |

### 対処方針

- **区切り文字**: 分割前に全角→半角の正規化を行う（例: `text.replace("；", ";")`）
- **前後の空白**: `.strip()` に加えて全角スペース `　` も除去する
- **文字列比較**: 比較前に両辺を正規化するか、正規化済みデータを保存する
