...
- **ADR（アーキテクチャ決定記録）**: 設計判断の「なぜ」を記録（全16本: `docs/ADR/`）
  ...
  - [ADR 0005: 抽出層とGold Layer分離（5層構成）](docs/ADR/0005-extraction-layer-gold-layer-separation.md)
  - [ADR 0007: リポジトリモデルパターン標準化](docs/ADR/0007-repository-model-pattern-standardization.md)
- **運用ガイド**: `docs/guides/` に格納（CICD, DEPLOYMENT, OPERATIONS, TROUBLESHOOTING）
- **監視設定**: `docs/monitoring/` に格納（Grafana, OpenTelemetry）
...
詳細は `just bi-dashboard` コマンドで起動できます。
...
> 📚 **アーキテクチャ図**: [docs/diagrams/](docs/diagrams/) にすべてのアーキテクチャ図があります
> 📖 **設計判断記録**: [docs/ADR/](docs/ADR/) で全16本のArchitecture Decision Recordsを参照

### Clean Architecture

Sagebaseは、保守性と拡張性を向上させるためClean Architectureを採用しています。

**移行状況**: 🟢 **100%完了** - コアアーキテクチャは完全に実装されています。
...
- [ADR 0001: Clean Architecture採用](docs/ADR/0001-clean-architecture-adoption.md) - 採用の背景と設計判断
