...
class TestExpandGroupJudgesPreview:
    ...
    @pytest.fixture
    def sample_proposal(self):
        return Proposal(id=100, title="テスト議案", meeting_id=200)

    @pytest.fixture
    def sample_meeting(self):
        return Meeting(id=200, conference_id=300, date=date(2025, 6, 15))
    ...
    async def test_preview_success(
        ...
        mock_politician_repo.get_by_ids.return_value = sample_politicians

        result = await use_case.preview(proposal_id=100, group_judge_ids=[1])
    ...
    async def test_preview_with_existing_votes(
        ...
        """既存投票データがある場合、has_existing_voteがTrueになる."""
        mock_proposal_repo.get_by_id.return_value = sample_proposal
        mock_meeting_repo.get_by_id.return_value = sample_meeting
        mock_group_judge_repo.get_by_proposal.return_value = [sample_group_judge]

        existing_judge = ProposalJudge(
            id=1, proposal_id=100, politician_id=501, approve="賛成"
        )
        mock_proposal_judge_repo.get_by_proposal.return_value = [existing_judge]
        mock_membership_repo.get_active_by_group.side_effect = [
            sample_memberships,
            [],
        ]
        ...
        member_501 = next(m for m in item.members if m.politician_id == 501)
        assert member_501.has_existing_vote is True

        member_502 = next(m for m in item.members if m.politician_id == 502)
        assert member_502.has_existing_vote is False

    async def test_preview_proposal_not_found(self, use_case, mock_proposal_repo):
        """議案が見つからない場合、エラーを返す."""
        mock_proposal_repo.get_by_id.return_value = None

        result = await use_case.preview(proposal_id=999, group_judge_ids=[1])
    ...
    async def test_preview_no_meeting_date(
        ...
        proposal = Proposal(id=100, title="テスト", meeting_id=None)
        mock_proposal_repo.get_by_id.return_value = proposal
        mock_group_judge_repo.get_by_proposal.return_value = [sample_group_judge]
        mock_proposal_judge_repo.get_by_proposal.return_value = []
    ...
    async def test_preview_no_matching_group_judges(
        ...
        result = await use_case.preview(proposal_id=100, group_judge_ids=[999])
