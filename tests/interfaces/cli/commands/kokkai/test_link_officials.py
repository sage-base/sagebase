...
class TestLinkOfficialsCommand:
    def test_dry_run_shows_candidates(self, mock_container: MagicMock) -> None:
    ...
    def test_execute_calls_usecase(self, mock_container: MagicMock) -> None:
        """本実行でUseCaseが呼ばれること."""
        mock_usecase = _setup_usecase_mock(mock_container)
        mock_usecase.execute = AsyncMock(
    ...
    def test_summary_displayed_correctly(self, mock_container: MagicMock) -> None:
        """結果サマリーが正しく表示されること."""
        mock_usecase = _setup_usecase_mock(mock_container)
        mock_usecase.execute = AsyncMock(
            return_value=_make_output(
    ...
    def test_no_details_hides_detail_section(self, mock_container: MagicMock) -> None:
