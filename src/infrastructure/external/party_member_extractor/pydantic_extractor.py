...
class PydanticPartyMemberExtractor(IPartyMemberExtractorService):
    """政党メンバー抽出器（Pydantic実装）

    Pydanticを使用して政党メンバー情報を抽出するクラス。
    従来の実装方式を Clean Architecture に準拠させたバージョン。
    """

    def __init__(self, llm_service: ILLMService) -> None:
        """Initialize PydanticPartyMemberExtractor
    ...
    async def extract_members(
        ...
        try:
            ...
            # LLMで議員情報を抽出（Pydantic使用）
            members_dto = await self._extract_members_with_pydantic(main_content, url)
            ...
            logger.error(f"Pydantic extraction error: {e}", exc_info=True)
            return PartyMemberExtractionResultDTO(
                party_id=party_id,
                url=url,
                extracted_members=[],
    ...
    async def _extract_members_with_pydantic(
        self, main_content: str, url: str
    ) -> list[ExtractedPartyMemberDTO]:
        """Pydanticを使用して議員情報を抽出する
        ...
        try:
            ...
            formatted_prompt = prompt.format(
                party_name=party_name, base_url=base_url, content=main_content
            )

            # LLMを使用して抽出
            logger.info("Calling LLM for party member extraction")
            result = self.extraction_llm.invoke(formatted_prompt)

            # Pydanticモデルの結果をDTOに変換
            members_dto = []
            ...
            logger.info(f"Pydantic extracted {len(members_dto)} members")
            return members_dto

        except Exception as e:
            logger.error(f"Pydantic extraction failed: {e}", exc_info=True)
            return []
    ...
    async def extract_from_html(
        ...
        try:
            ...
            # LLMで議員情報を抽出（Pydantic使用）
            members_dto = await self._extract_members_with_llm_for_party(
    ...
    async def _extract_members_with_llm_for_party(
        self, main_content: str, url: str, party_name: str
    ) -> list[ExtractedPartyMemberDTO]:
        """LLMを使用して議員情報を抽出する（政党名指定版）
        ...
        try:
            # LLM chainを作成して実行
            base_url = self._get_base_url(url)
            result = await self.llm_service.invoke_structured_output_async(
                ...
                output_model=PartyMemberList,  # type: ignore[reportArgumentType]
            )

            # Pydanticモデルの結果をDTOに変換
            members_dto = []
            for m in result.members:  # type: ignore[reportAttributeAccessIssue]
                profile_url = m.profile_url
                if profile_url and not profile_url.startswith("http"):
                    profile_url = base_url + profile_url.lstrip("/")
    ...
    async def _extract_from_pages_async(
        ...
        logger.info(
            f"Starting Pydantic extraction from {len(pages)} pages for {party_name}"
        )
